...
@section('title','Edit BillingService')
@section('style')
...
<form action="{{route('billingservices.update',$billingservice)}}" method ="post" enctype="multipart/form-data">
@csrf
@method("PUT")
<div class="row mb-3">
	...
	<div class="col-sm-10">
		<select class="form-control" name="billing_id" id="billing_id">
			@foreach($billings as $billing)
				@if($billing->id==$billingservice->billing_id)
	...
	<div class="col-sm-10">
		<select class="form-control" name="service_id" id="service_id">
			@foreach($services as $service)
				@if($service->id==$billingservice->service_id)
...
<div class="row mb-3">
	...
	<div class="col-sm-10">
		<input type = "text" class="form-control" name="price" value="{{$billingservice->price}}" id="price" placeholder="Price">
	</div>
</div>
        ...
        <div class="col-md-11 col-lg-9 col-12 mt-3 fw-bold fs-4 mx-auto">
            <div class="card mb-4">
                <div class="card-header bg-primary text-white d-flex justify-content-between align-items-center">
                    <h3 class="mb-0 fw-bolder fs-2">Edit Billing Service Details</h3>
                    ...
                    <form action="{{ route('billingservices.update', $billingservice) }}" method="post" enctype="multipart/form-data">
                        ...
                        <div class="mb-1">
                            ...
                            <select class="form-select form-control-lg" name="billing_id" id="billing_id">
                                ...
                                @foreach($billings as $billing)
                                    <option value="{{ $billing->id }}" {{ old('billing_id', $billingservice->billing_id) == $billing->id ? 'selected' : '' }}>
                                        {{ $billing->id }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="mb-1">
                            ...
                            <select class="form-select form-control-lg" name="service_id" id="service_id">
                                ...
                                @foreach($services as $service)
                                    <option value="{{ $service->id }}" {{ old('service_id', $billingservice->service_id) == $service->id ? 'selected' : '' }}>
                                        {{ $service->name }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="mb-1">
                            ...
                            <input type="text" class="form-control form-control-lg" name="price" id="price"
                                placeholder="Enter Price" value="{{ old('price', $billingservice->price) }}" />
                            @error('price')
                                <span class="text-danger">{{ $message }}</span>
                            @enderror
                        </div>
